...
describe('API Tests', () => {
  ...
  describe('Login API', () => {
    ...
    it('should return 400 if login fails', async () => {
      ...
      const response = await loginHandler({ request, locals: { auth: { setSession: vi.fn() } } } as any);
      expect(response.status).toBe(400);
      expect(await response.json()).toEqual({ message: 'Invalid email or password' });
    });
  ...
  describe('Register API', () => {
    ...
    it('should return 400 if registration fails', async () => {
      ...
      const response = await registerHandler({ request, locals: { auth: { setSession: vi.fn() } } } as any);
      expect(response.status).toBe(400);
