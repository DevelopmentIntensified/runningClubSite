<script lang="ts">
  ...
  let filteredLinks = $derived.by(() => {
    ...
    return data.links.filter(
      (link: (typeof data.links)[0]) =>
        link.title.toLowerCase().includes(term) || link.season.toLowerCase().includes(term)
    );
  });
</script>
  ...
  <div class="mx-auto mb-8 max-w-md">
    <div class="relative">
      <input
        ...
        class="focus:border-primary-500 focus:ring-primary-500/20 w-full rounded-full border border-slate-200 bg-white px-5 py-3 pl-12 text-sm text-slate-700 shadow-sm focus:ring-2 focus:outline-none"
      />
      <svg
    ...
    <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
      {#each filteredLinks as link}
        ...
        >
          ...
          <div
            class="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-t from-black/70 via-black/20 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100"
          >
