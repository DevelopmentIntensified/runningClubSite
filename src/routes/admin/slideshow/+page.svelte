          ...
          {#each data.slideshowImages as image (image.id)}
            <tr class="hover:bg-slate-50/50 transition-colors">
              <td class="whitespace-nowrap px-6 py-4">
                <img src={image.imageUrl} alt={image.title} class="h-14 w-20 rounded-lg object-cover shadow-sm" />
              </td>
              <td class="whitespace-nowrap px-6 py-4 text-sm font-medium text-slate-700">{image.title}</td>
              <td class="whitespace-nowrap px-6 py-4 text-sm text-slate-600">{image.order}</td>
              <td class="whitespace-nowrap px-6 py-4 text-sm">
                <div class="flex items-center gap-3">
                  <a href="/admin/slideshow/edit/{image.id}" class="text-primary-600 hover:text-primary-800 font-medium text-xs">Edit</a>
