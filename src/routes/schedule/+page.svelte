<script lang="ts">
  ...
  import { downloadSchoolYearEventsICS, type CalendarEvent } from '$lib/utils/calendarExport';
  ...
  function handleDownloadSchoolYear() {
    const calendarEvents: CalendarEvent[] = (data.events as ScheduleEventRow[]).map((e) => ({
      id: e.id,
      title: e.title,
    ...
    downloadSchoolYearEventsICS(calendarEvents);
  }
</script>
